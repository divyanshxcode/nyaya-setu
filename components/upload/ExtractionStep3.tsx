      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.6fr)_minmax(320px,0.9fr)]">
        <div className="space-y-5">
          <Card className="border-slate-200 shadow-sm">
            ...
            <CardContent>
              <p className="text-sm leading-7 text-foreground">{actionPlan.executiveSummary}</p>
            </CardContent>
          </Card>
          ...
          <Card className="border-slate-200 shadow-sm">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between gap-3">
                <SectionTitle
                  title="Action Items"
                  count={actionPlan.actionItems.length}
                  accent="bg-teal-600"
                />
            ...
            <Card className="border-slate-200 shadow-sm">
              <CardHeader className="pb-3">
                <SectionTitle
                  ...
                  count={actionPlan.complianceRequirements.length}
                  accent="bg-blue-600"
                />
              </CardHeader>
              <CardContent className="grid gap-3 md:grid-cols-2">
                {actionPlan.complianceRequirements.map((req) => (
                  <div key={req.id} className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                    ...
                    {req.legalBasis && (
                      <p className="mt-2 text-sm leading-6 text-muted-foreground">{req.legalBasis}</p>
                    )}
          ...
          {actionPlan.keyTimelines.length > 0 && (
            <Card className="border-slate-200 shadow-sm">
              <CardHeader className="pb-3">
                <SectionTitle
                  title="Key Timelines"
                  count={actionPlan.keyTimelines.length}
                  accent="bg-green-600"
                />
              </CardHeader>
              <CardContent className="space-y-3">
                {actionPlan.keyTimelines.map((timeline, idx) => (
                  <div key={idx} className="flex gap-4 rounded-lg border border-slate-200 p-3">
                    <div className="min-w-28 text-sm font-semibold text-foreground">
                      {formatDate(timeline.date)}
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="text-sm text-foreground">{timeline.description}</p>
                      <p className="mt-1 text-xs capitalize text-muted-foreground">
                        {timeline.isExplicit ? 'Explicit' : 'Inferred'} • {timeline.type}
                      </p>
                    </div>
                  </div>
                ))}
          ...
          <Card className="border-slate-200 shadow-sm xl:sticky xl:top-6">
            ...
            <CardContent className="space-y-5">
              {actionPlan.appealConsiderations && (
                <div className="space-y-3">
                  <SectionTitle title="Appeal Considerations" accent="bg-violet-600" />
                  <Badge
              ...
              {actionPlan.riskAssessment && (
                <div className="space-y-3">
                  <SectionTitle title="Risk Assessment" accent="bg-red-600" />
              ...
              {actionPlan.recommendedDocumentation.length > 0 && (
                <div className="space-y-3">
                  <SectionTitle
                    title="Documentation"
                    count={actionPlan.recommendedDocumentation.length}
              ...
              {actionPlan.externalCounselInstructions.length > 0 && (
                <div className="space-y-3">
                  <SectionTitle
                    ...
                    count={actionPlan.externalCounselInstructions.length}
                    accent="bg-amber-600"
                  />
              ...
              {actionPlan.internalCommunicationPlan.length > 0 && (
                <div className="space-y-3">
                  <SectionTitle
                    ...
                    count={actionPlan.internalCommunicationPlan.length}
                    accent="bg-cyan-600"
                  />
